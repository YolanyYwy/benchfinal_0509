...
from AGentCL.domains.ota.data_model import (
    OTADB, Order, Hotel, Attraction, Flight, Train, 
    HotelProduct, AttractionProduct, FlightProduct, TrainProduct
)
...
def rerank(query: str, candidates: Dict[str, str], top_k: int = 100) -> List[str]:
    """Rerank candidates based on query keywords."""
...
class OTATools(ToolKitBase):
    ...
    @is_tool(tool_type=ToolType.READ)
    def search_hotel_order(self, user_id: str, date: Optional[str] = None, status: Optional[str] = "paid") -> str:
