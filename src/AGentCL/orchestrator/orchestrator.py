...
class Orchestrator:
    ...
    def run(self) -> SimulationRun:
        ...
        self._print_trajectory_summary(simulation_run)

        return simulation_run

    def _print_trajectory_summary(self, simulation_run: SimulationRun):
        """Print a summary of the trajectory with success/failure status."""
        term_reason = simulation_run.termination_reason
        ...
        is_success = term_reason in success_reasons

        status = "✓" if is_success else "✗"
        status_text = "SUCCESS" if is_success else "FAILED"

        print(f"\n{'='*60}")
        print(f"[{status}] Trajectory {status_text}")
        print(f"{'='*60}")
        ...
        if simulation_run.agent_cost is not None:
            print(f"  Agent Cost: ${simulation_run.agent_cost:.4f}")
        if simulation_run.user_cost is not None:
            print(f"  User Cost: ${simulation_run.user_cost:.4f}")

        # Print message breakdown
        assistant_msgs = sum(1 for m in simulation_run.messages if m.role == "assistant")
        user_msgs = sum(1 for m in simulation_run.messages if m.role == "user")
        tool_msgs = sum(1 for m in simulation_run.messages if m.role == "tool")

        print(f"  Message Breakdown: Assistant={assistant_msgs}, User={user_msgs}, Tool={tool_msgs}")
        print(f"{'='*60}\n")
