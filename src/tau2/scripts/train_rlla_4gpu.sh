#!/usr/bin/env bash
# ToolRL-style GRPO training on rlla_by_domain data (4-GPU DeepSpeed ZeRO-2)
#
# Usage:
#   bash train_rlla_4gpu.sh                          # train all domains
#   DOMAIN=airline bash train_rlla_4gpu.sh           # single domain
#   DOMAIN=toolrl_4k NUM_STEPS=200 bash train_rlla_4gpu.sh
#
# 断点续训:
#   DOMAIN=airline RESUME_FROM_CHECKPOINT=logs/rlla/checkpoint_task_0_step_100 bash train_rlla_4gpu.sh
...
ACCELERATE_CONFIG=${ACCELERATE_CONFIG:-"/home/haoxiangzhao/ywy/AGentCL/src/tau2/continual_learning/continual_learning/accelerate_4gpu_bf16.yaml"}
...
# ========== vLLM ==========
VLLM_TP=${VLLM_TP:-1}
VLLM_MEM=${VLLM_MEM:-0.3}    # 4卡每卡显存更充裕，给 vLLM 更多
...
# ========== 要训练的 domain ==========
ALL_DOMAINS="toolrl_4k airline retail api_bank bamboogle"
